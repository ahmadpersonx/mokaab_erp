...
import 'package:mokaab_erp/features/finance/models/cost_center_model.dart';
...
class FinanceService {
  ...
  Future<List<Account>> getCustomerAccounts() async {
    try {
      // ✅ تم التحديث: حساب العملاء الرئيسي رمزه '1231'
      // نجلب كل الحسابات التي تبدأ بهذا الرمز وتكون قابلة للترحيل
      final response = await supabase
          .from('accounts')
          .select()
          .ilike('code', '1231%') // 'ilike' for case-insensitive search
  ...
  Future<List<Account>> getCashBoxAccounts() async {
    try {
      final response = await supabase
          .from('accounts')
          .select()
          .ilike('code', '1211%') // ✅ تم التحديث: الخزينة تبدأ بـ 1211
  ...
  Future<List<Account>> getBankAccounts() async {
    try {
      final response = await supabase
          .from('accounts')
          .select()
          .ilike('code', '1212%') // ✅ تم التحديث: البنوك تبدأ بـ 1212
          .eq('is_transaction', true);
      return (response as List)
          .map((acc) => Account.fromJson(acc))
          .toList();
